...
export function MessagesPage() {
  const conversations: DummyConversation[] = useMemo(
    () => [
      ...
      { id: 'c3', name: 'Liam Hossain', context: 'vendor', lastMessage: 'What’s the ETA for delivery?' },
    ],
    [],
  )
  ...
  const [messagesByConv, setMessagesByConv] = useState<Record<string, DummyMessage[]>>(() => ({
    ...
    c2: [{ id: 'm3', sender: 'them', text: 'Can you share the requirements doc?', createdAt: new Date(Date.now() - 1000 * 60 * 10).toISOString() }],
    c3: [{ id: 'm4', sender: 'them', text: 'What’s the ETA for delivery?', createdAt: new Date(Date.now() - 1000 * 60 * 5).toISOString() }],
  }))
