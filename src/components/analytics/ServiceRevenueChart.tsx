...
import type { AnalyticsRangeKey, AnalyticsRevenuePoint } from '@/features/api/types'
    ...
    <Card className={cn('rounded-xl border-border/60 shadow-sm', className)}>
      <CardHeader className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
        <div className="space-y-1">
          ...
          <CardDescription>Service earnings (approved only) and bookings over time.</CardDescription>
        </div>
      ...
      <CardContent>
        ...
        {hovered ? (
          <div className="text-muted-foreground mt-3 text-xs">
            ...
            service earnings · <span className="text-foreground font-medium">{hovered.ordersJobs}</span> bookings
          </div>
        ) : null}
      </CardContent>
    </Card>
